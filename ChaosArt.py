...
START_POLYGON = SQUARE
ATTRACTIVE = START_POLYGON + [(0, 0)]
GAME = ChaosGame
rule = "no repeats"
...
SAVE_IMAGE = True
SAVE_FILE_PATH = "Images/Chaos_Game/" + "test" + ".jpg"

my_dpi = 100
plt.figure(figsize=(800 / my_dpi, 800 / my_dpi), dpi=my_dpi)
plt.axis("off")
plt.title("Starting Shape: " + "Trapezoid" + ", Jump: " + str(round(JUMP, 3)) + ", Special Rules: " +
          (rule if GAME.__name__.__contains__("Special") else "None"))
...
def chaos_game(attractive_points, iterations=5000, draw_triangle=False):
    ...
    if GAME.__name__.__contains__("Special"):
        cg = GAME(attractive_points, jump=JUMP, start_polygon=START_POLYGON, rule=rule)
    else:
        cg = GAME(attractive_points, start_polygon=START_POLYGON, jump=JUMP)
